...
def update_model(endpoint):
    endpoint_model_map = {
        "Groq": "llama3-70b-8192", # llama-3.1-8b-instant
        "OpenAI": "gpt-3.5-turbo", # "gpt-3.5-turbo", "gpt-4o"
...
# def evaluate_translation(final_translation: str, reference: str, target_lang: str):
#     if not reference:
#         raise gr.Error("Please provide a reference translation for evaluation")
        
#     return calculate_metrics(reference, final_translation,source_lang,target_lang)
def evaluate_translation(final_translation: str, reference: str, target_lang: str):
    if not reference:
        raise gr.Error("Please provide a reference translation for evaluation")
    # try:
    metrics = calculate_metrics(reference, final_translation, source_lang, target_lang)
        # plot = create_error_plot(metrics['error_types'])
    return metrics, metrics['error_plot'] #plot
    # except Exception as e:
...
with gr.Blocks(theme="soft", css=CSS, fill_height=True) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1) as menubar:
            endpoint = gr.Dropdown(
                label="Endpoint",
                choices=["OpenAI", "Groq", "DeepSeek-r1"], #, "TogetherAI", "Ollama", "CUSTOM"
                value="OpenAI",
            )
            choice = gr.Checkbox(
            ...
            model = gr.Textbox(
                label="Model",
                value="gpt-3.5-turbo", # "gpt-3.5-turbo", "gpt-4o"
            )
            api_key = gr.Textbox(
                label="API_KEY",
                type="password",
            ...
            # eval_button = gr.Button("Evaluate Quality")
            switch_btn = gr.Button(value="🔄️")
            country = gr.Textbox(
                label="Country", value="Lebanon", max_lines=1
            )
        ...
        with gr.Column(scale=4):
            source_text = gr.Textbox(
                label="Source Text",
                value="my grandmother visited us yesterday",
                lines=12,
            )
            with gr.Tab("Final"):
            ...
            #     reference_translation = gr.Textbox(label="Reference Translation", lines=12)
    ...
    # Evaluation Section
        ...
        with gr.Column(scale=4):
            with gr.Row():
                reference_translation = gr.Textbox(
                    label="Reference Translation",
                    lines=6,
                    scale=8,
                    placeholder="Paste reference translation here..."
                )
            with gr.Row():
            ...
            with gr.Row():
                with gr.Column(scale=6):
                    eval_metrics = gr.JSON(
                        label="Quality Metrics",
                        elem_classes="metrics-container",
                        show_label=True
                    )
                with gr.Column(scale=4):
                    error_plot = gr.Plot(
    ...
    eval_button.click(
        ...
        inputs=[output_final, reference_translation, target_lang],
        outputs=[eval_metrics, error_plot] 
    )
    menu_btn.click(
